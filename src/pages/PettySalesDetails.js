...
function PettySalesDetails() {
  ...
  // Fetch sales details from API using direct endpoint
  useEffect(() => {
    const fetchSalesData = async () => {
      try {
        ...
        console.log(`🔍 Fetching sales details for ID: ${id}`);
        
        // Use the direct GET endpoint for single sales entry
        const response = await fetch(`${BASE_URL}/api/cash/sales/${id}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
        ...
        if (!response.ok) {
          if (response.status === 404) {
            throw new Error(`Sales entry with ID ${id} not found`);
          }
          throw new Error(`Failed to fetch sales data: ${response.status}`);
        }

        const result = await response.json();
        console.log('✅ Sales entry fetched:', result);
  ...
  const handleDownloadFile = () => {
    if (salesData?.file_path) {
      window.open(salesData.file_path, '_blank');
    }
  };

  // Get creator name from the creator object
  const getCreatorName = () => {
    if (salesData?.creator) {
      return salesData.creator.username || salesData.creator.email || `User ${salesData.created_by}`;
    }
    return `User ${salesData?.created_by || 'N/A'}`;
  };

  // Get customer details from job_card object
  const getCustomerDetails = () => {
    if (salesData?.job_card) {
      return {
        name: salesData.job_card.customer_name || 'N/A',
        email: salesData.job_card.customer_email || 'N/A',
        vehicle: salesData.job_card.vehicle_number || 'N/A',
        carMake: salesData.job_card.car_make || 'N/A',
        carModel: salesData.job_card.car_model || 'N/A'
      };
    }
    return {
      name: 'N/A',
      email: 'N/A',
      vehicle: salesData?.vehicle_number || 'N/A',
      carMake: 'N/A',
      carModel: 'N/A'
    };
  };
  ...
  if (error || !salesData) {
    return (
      <Box sx={{ p: 3 }}>
        <Button startIcon={<ArrowBackIcon />} onClick={() => navigate('/sales-cash')}>
          Back to Sales
        </Button>
        <Alert severity="error" sx={{ mt: 2 }}>
          {error || 'Sales entry not found'}
      ...
      <Container maxWidth="lg">
            ...
            {isAccountsDept && (
              <>
                <Button 
                  ...
                  variant="outlined"
                  color="primary"
                  onClick={() => navigate(`/sales-cash/edit/${salesData.id}`)}
                >
                  Edit
                </Button>
                <Button 
              ...
              <CardContent sx={{ p: 4 }}>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 4 }}>
                  <Box>
                    ...
                    <Typography variant="h6" color="textSecondary">
                      Sales Entry #S-{salesData.id?.toString().padStart(4, '0') || 'N/A'}
                    </Typography>
                    <Chip 
                      label={getStatusDisplay(salesData.status)} 
                      color={getStatusColor(salesData.status)}
                      size="medium"
                      sx={{ mt: 1 }}
                    />
                  </Box>
                  ...
                  <Grid item xs={12} md={6}>
                    <Paper sx={{ p: 3, backgroundColor: '#e3f2fd', color: '#1565c0' }}>
                      ...
                      <Box sx={{ mt: 2 }}>
                        <Typography variant="body1" gutterBottom sx={{ display: 'flex', alignItems: 'center' }}>
                          <WalletIcon sx={{ mr: 1, fontSize: 20 }} />
                          <strong>Amount:</strong> {formatCurrency(salesData.amount)}
                        </Typography>
                        <Typography variant="body1" gutterBottom sx={{ display: 'flex', alignItems: 'center' }}>
                          <LocalAtmIcon sx={{ mr: 1, fontSize: 20 }} />
                          <strong>Payment Mode:</strong> {salesData.mode_of_payment || 'N/A'}
                        </Typography>
                        <Typography variant="body1" gutterBottom sx={{ display: 'flex', alignItems: 'center' }}>
                          <ReceiptIcon sx={{ mr: 1, fontSize: 20 }} />
                          <strong>Transaction #:</strong> {salesData.transaction_number || 'N/A'}
                        </Typography>
                        <Typography variant="body1" gutterBottom sx={{ display: 'flex', alignItems: 'center' }}>
                          <CalendarIcon sx={{ mr: 1, fontSize: 20 }} />
                          <strong>Date:</strong> {formatDate(salesData.created_at)}
                        </Typography>
                      </Box>
                    </Paper>
                  </Grid>
                  ...
                  <Grid item xs={12} md={6}>
                    <Paper sx={{ p: 3, backgroundColor: '#fce4ec', color: '#ad1457' }}>
                      ...
                      <Box sx={{ mt: 2 }}>
                        ...
                        <Typography variant="body1" gutterBottom sx={{ display: 'flex', alignItems: 'center' }}>
                          <CarRepairIcon sx={{ mr: 1, fontSize: 20 }} />
                          <strong>Vehicle:</strong> {customerDetails.vehicle}
                        </Typography>
                        <Typography variant="body1" gutterBottom>
                          <strong>Car:</strong> {customerDetails.carMake} {customerDetails.carModel}
                        </Typography>
                      </Box>
                    </Paper>
                  </Grid>
                ...
                <Paper sx={{ p: 3, mt: 3, backgroundColor: '#e8f5e8' }}>
                  ...
                  <Box sx={{ mt: 2 }}>
                    <Grid container spacing={2}>
                      <Grid item xs={12} md={6}>
                        <Typography variant="body1" gutterBottom>
                          <strong>Job Card ID:</strong> {salesData.job_card_id || 'N/A'}
                        </Typography>
                        <Typography variant="body1" gutterBottom>
                          <strong>Job Card Number:</strong> {salesData.job_card_number || 'N/A'}
                        </Typography>
                      </Grid>
                      <Grid item xs={12} md={6}>
                        <Typography variant="body1" gutterBottom>
                          <strong>Vehicle Number:</strong> {salesData.vehicle_number || 'N/A'}
                        </Typography>
                        {salesData.job_card_id && (
                          <Button 
                            variant="outlined" 
                            color="primary"
                            startIcon={<CarRepairIcon />}
                            onClick={() => navigate(`/job-cards/view/${salesData.job_card_id}`)}
                            sx={{ mt: 1 }}
                          >
                ...
                {salesData.file_path && (
                  <Paper sx={{ p: 3, mt: 3, backgroundColor: '#fff3e0' }}>
                    <Typography variant="h6" gutterBottom color="#ef6c00" fontWeight="bold">
                      📎 Attached File
                    </Typography>
                    <Box sx={{ mt: 2 }}>
                      <List>
                        <ListItem>
                          ...
                          <ListItemText 
                            primary="Receipt/Invoice"
                            secondary="Supporting document for this sales entry"
                          />
                          <Button 
                            variant="contained"
                            startIcon={<AttachFileIcon />}
                ...
                <Paper sx={{ p: 3, mt: 3, backgroundColor: '#f3e5f5' }}>
                  <Typography variant="h6" gutterBottom color="#7b1fa2" fontWeight="bold">
                    📅 Transaction Timeline
                  </Typography>
                  <Box sx={{ mt: 2 }}>
                    <List>
                      <ListItem>
                        ...
                        <ListItemText 
                          primary="Entry Created" 
                          secondary={formatDate(salesData.created_at)}
                        />
                      </ListItem>
                      <ListItem>
                        <ListItemIcon>
                        ...
                        <ListItemText 
                          primary="Last Updated" 
                          secondary={formatDate(salesData.updated_at)}
                        />
                      </ListItem>
                    </List>
                  </Box>
          ...
          <Grid item xs={12} lg={4}>
              ...
              <CardContent sx={{ p: 3 }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', py: 2 }}>
                  ...
                  <Box>
                    ...
                    {salesData.creator?.email && (
                      <Typography variant="body2" color="textSecondary">
                        {salesData.creator.email}
                      </Typography>
                    )}
                    <Typography variant="caption" color="textSecondary">
                      Created on {formatDate(salesData.created_at)}
                    </Typography>
                  </Box>
                </Box>
              </CardContent>
            ...
            <Card sx={{ boxShadow: 3 }}>
              <CardContent sx={{ p: 3 }}>
                ...
                <Box sx={{ mt: 2 }}>
                  <Typography variant="body2" gutterBottom>
                    <strong>Sales ID:</strong> {salesData.id || 'N/A'}
                  </Typography>
                  <Typography variant="body2" gutterBottom>
                    <strong>Vehicle Number:</strong> {salesData.vehicle_number || 'N/A'}
                  </Typography>
                  ...
                  <Typography variant="body2" gutterBottom>
                    <strong>Created:</strong> {formatDate(salesData.created_at)}
                  </Typography>
                  <Typography variant="body2" gutterBottom>
                    <strong>Last Updated:</strong> {formatDate(salesData.updated_at)}
                  </Typography>
                  <Typography variant="body2" gutterBottom>
                    <strong>Created By:</strong> {getCreatorName()}
                  </Typography>
                </Box>
              </CardContent>
            </Card>
        ...
        <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)} maxWidth="sm" fullWidth>
          ...
          <DialogContent>
            ...
            <Alert severity="warning" sx={{ mt: 2 }}>
              <strong>Warning:</strong> This action cannot be undone. The sales entry for{' '}
              <strong>{formatCurrency(salesData.amount)}</strong> will be permanently deleted.
            </Alert>
          </DialogContent>
          <DialogActions>
            <Button onClick={() => setDeleteDialogOpen(false)} disabled={deleteLoading}>
