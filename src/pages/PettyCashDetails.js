...
function PettyCashDetails() {
  ...
  // Fetch petty cash details from API using direct endpoint
  useEffect(() => {
    const fetchPettyCash = async () => {
      try {
        setLoading(true);
        ...
        console.log(`🔍 Fetching petty cash details for ID: ${id}`);
        
        // Use the direct GET endpoint for single petty cash entry
        const response = await fetch(`${BASE_URL}/api/cash/petty/${id}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
        ...
        const result = await response.json();
        console.log('✅ Petty cash entry fetched:', result);
  ...
  const handleDelete = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Navigate back to petty cash list
        navigate('/petty-cash');
      } else {
        throw new Error('Failed to delete petty cash entry');
      }
  ...
  const handleDownloadFile = () => {
    if (pettyCash?.file_path) {
      window.open(pettyCash.file_path, '_blank');
    }
  };

  // Get creator name from the creator object
  const getCreatorName = () => {
    if (pettyCash?.creator) {
      return pettyCash.creator.username || pettyCash.creator.email || `User ${pettyCash.created_by}`;
    }
    return `User ${pettyCash?.created_by || 'N/A'}`;
  };
  ...
  if (error || !pettyCash) {
    return (
      <Box sx={{ p: 3 }}>
        <Button startIcon={<ArrowBackIcon />} onClick={() => navigate('/petty-cash')}>
          Back to Petty Cash
        </Button>
        <Alert severity="error" sx={{ mt: 2 }}>
          {error || 'Petty cash entry not found'}
                      ...
                      <Box sx={{ mt: 2 }}>
                        ...
                        {pettyCash.job_card_id && (
                          <Button 
                            ...
                            onClick={() => navigate(`/job-cards/view/${pettyCash.job_card_id}`)}
                          >
                            View Job Card
                          </Button>
                        )}
              ...
              <CardContent sx={{ p: 3 }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', py: 2 }}>
                  ...
                  <Box>
                    ...
                    {pettyCash.creator?.email && (
                      <Typography variant="body2" color="textSecondary">
                        {pettyCash.creator.email}
                      </Typography>
                    )}
                    <Typography variant="caption" color="textSecondary">
                      Created on {formatDate(pettyCash.created_at)}
                    </Typography>
                  </Box>
                </Box>
              </CardContent>
            ...
            <Card sx={{ boxShadow: 3 }}>
              <CardContent sx={{ p: 3 }}>
                ...
                <Box sx={{ mt: 2 }}>
                  ...
                  <Typography variant="body2" gutterBottom>
                    <strong>Job Card ID:</strong> {pettyCash.job_card_id || 'N/A'}
                  </Typography>
                  <Typography variant="body2" gutterBottom>
                    <strong>Created:</strong> {formatDate(pettyCash.created_at)}
                  </Typography>
